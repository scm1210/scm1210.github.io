...
df <- df %>% 
  ...
  filter(bin < 5) |> 
  mutate(is_Preamble = factor(is_Preamble, levels = c(0, 1), labels = c("Not Preamble", "Preamble"))) |> 
  filter(!(is_Preamble == "Preamble" & bin == 1))
...
data_plot_good |> 
  ggplot(aes(x = daysSinceFirstText, y = distance, group = user_id, color = word_attribute)) +
  geom_line(alpha = 0.1, color = "gray") +  # Individual lines
  geom_smooth(aes(group = word_attribute), method = "gam", se = T, size = 1.2) +  # Omnibus trend lines
  labs(title = "Individual Trajectories with GAM Curve",
       x = "Days in Therapy", y = "i_MADS") +
  plot_aes
```
