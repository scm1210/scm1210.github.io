---
title: "I representation: Compiled Results"
author: "Steven"
date: "`r Sys.Date()`"
format: 
  revealjs:
    ...
    smaller: true
    #slide-number: true 
    controls: true                 # Show next/prev arrows for easier navigation
    progress: true                 # Show a progress bar at the bottom
    keyboard: true                 # Enable keyboard navigation (usually on by default)
    slideNumberFormat: "%current% / %total%"  # More informative slide numbering
    navigationMode: 'linear'       # Ensures left/right arrows move strictly in order
    transition: "slide"            # Slide transition effect (could be 'fade', 'convex', 'concave', 'zoom')
    transitionSpeed: "fast"        # Transition speed: 'default', 'fast', or 'slow'
    touch: true                   # Enable touch gestures for mobile/tablets  
---
...
#df = read_csv('/Users/sm9518/Library/CloudStorage/Box-Box/Talkspace_ErikNook_HarvardPrinceton/Data/Processed/Mesquiti_LBA_2024/Talkspace-LBA/I-representations/data/text-level/Talkspace_MADS_EXP_text.csv')

df = read_csv('/Users/sm9518/Library/CloudStorage/Box-Box/Talkspace_ErikNook_HarvardPrinceton/Data/Processed/Mesquiti_LBA_2024/Talkspace-LBA/I-representations/data/text-level/Talkspace_MADS_EXP_text_States.csv')

# Step 2: Clean and filter main dataframe
df <- df %>% 
  rename(
  ...
  filter(bin < 5) %>% 
  mutate(
    Llama_rating = ifelse(is.na(Llama_rating), 0, Llama_rating),
    Llama_rating = factor(Llama_rating, levels = c(0, 1), labels = c("Not Preamble", "Preamble"))
  ) %>% 
  # Now filter preambles from bin 1
  filter(!(Llama_rating == "Preamble"))

# Step 3: Average by daysSinceFirstText
df_avg_by_day <- df %>%
...
good_bad_data_small <- df_avg_by_day |> 
  mutate(
    i_MADS = i_good - i_bad,
  )  |> 
  filter(
...
able_unable_data_small <- df_avg_by_day |> 
  mutate(
...
disaggregated_able_unable <- within |> 
  left_join(between, by = "user_id") |> 
  select(1:14,daysSinceFirstText,
...
data_plot_good |> 
 ...
 annotate(
    ...
    y = .027,  # Near the top of the plot
  ...
  annotate(
   ...
   #annotate("text", x = 5, y = 0.025, label = "Closer to Good →", hjust = 0, size = 5, fontface = "italic",angle = 90) +
  #annotate("text", x =  5, y = 0.02, label = "← Closer to Bad", hjust = 1, size = 5, fontface = "italic",angle = 90) 
```

```{r}
good_bad_data_small$days_c <- good_bad_data_small$daysSinceFirstText - mean(good_bad_data_small$daysSinceFirstText, na.rm = TRUE)
good_bad_data_small$days_c2 <- good_bad_data_small$days_c^2

i_MADS_change <- lmer(
  i_MADS ~ days_c + days_c2 + (1 | room_id),
  data = good_bad_data_small
)

summary(i_MADS_change)
```

```{r}
# checking to see if the preamble texts have higher mads 
library(ggdist)
library(effectsize)

states = read_csv('/Users/sm9518/Library/CloudStorage/Box-Box/Talkspace_ErikNook_HarvardPrinceton/Data/Processed/Mesquiti_LBA_2024/Talkspace-LBA/I-representations/data/text-level/Talkspace_MADS_EXP_text_States.csv')
...
states %>%
  filter(bin == 1) %>%
  mutate( Llama_rating = ifelse(is.na(Llama_rating), 0, Llama_rating),
    is_Preamble = factor(Llama_rating, levels = c(0, 1), labels = c("Not Preamble", "Preamble"))) %>%
  { 
    df <- .
    ...
    # Updated factor labels with Ns
    df$is_Preamble <- factor(
      df$is_Preamble,
      levels = c("Not Preamble", "Preamble"),
      labels = paste0(c("Not Preamble", "Preamble"), " (n=", group_ns, ")")
    )
    
    # Run t-test and effect size
    t_res <- t.test(i_MADS ~ is_Preamble, data = df)
    d_res <- cohens_d(i_MADS ~ is_Preamble, data = df)

    # Plot
    ggplot(df, aes(x = is_Preamble, y = i_MADS, fill = is_Preamble)) +
      stat_halfeye(
        adjust = 0.5,
        width = 0.6,
        .width = 0,
      ...
      labs(
        x = "",
        y = "i_MADS",
        title = "Raincloud Plot of i_MADS by Preamble Status (bin 1)",
        subtitle = paste0(
          "t(", round(t_res$parameter, 1), ") = ",
          round(t_res$statistic, 2),
          ", p = ", signif(t_res$p.value, 3),
          "; d = ", round(d_res$Cohens_d, 2)
        )
      ) + plot_aes
  }
...
data_plot_able |> 
  filter(word_attribute == "i_MADS") |>
  ggplot(aes(x = daysSinceFirstText, y = distance)) +
  geom_smooth(method = "lm", se = TRUE, size = 1.2, color = able_palette[2]) +  # Linear trend
  scale_y_continuous(
    limits = c(min(data_plot_able$distance, na.rm = TRUE), 0.06),  # Add more headroom
    expand = expansion(mult = c(0.03, 0.1))  # Padding at top/bottom
  ) +
  labs(y = "Mean Attribute\nDifference Score", 
       x = "Days in Therapy") + 
  plot_aes +  
  ...
  annotate(
    "text",
    x = max(data_sx_able$daysSinceFirstText, na.rm = TRUE) * 0.75,
    y = 0.055,  # Top annotation
    label = annotation_text,
    color = "black",
    size = 6,
    fontface = "bold",
  ...
  annotate(
    "text",
    x = mean(data_sx_able$daysSinceFirstText, na.rm = TRUE),
    y = 0.055,
    label = sig_label,
    color = "black",
    size = 6,
  ...
  annotate("text", x = 3, y = 0.04, label = "Closer to Able →", hjust = 0, size = 5, fontface = "italic", angle = 90) +
  annotate("text", x = 3, y = 0.03, label = "← Closer to Unable", hjust = 1, size = 5, fontface = "italic", angle = 90)
...
data_small <- df_avg_by_day |> 
  mutate(
    the_good = the_good - the_bad,
    the_able = the_able - the_unable,
    and_good = and_good - and_bad,
    and_able = and_able - and_unable,
    to_good = to_good - to_bad,
    to_able = to_able - to_unable,
    be_good = be_good - be_bad,
    be_able = be_able - be_unable,
    a_good = a_good - a_bad,
    a_able = a_able - a_unable,
    we_good = we_good - we_bad,
    we_able = we_able - we_unable,
    Internalizing = GAD + PHQ
  ) 
...
between <- data_small %>%
  gather(item, value,
         the_good, and_good, to_good, be_good, a_good, we_good,
         the_able, and_able, to_able, be_able, a_able, we_able) %>%
  select(user_id, item, value) %>%
  unique() %>%
  group_by(user_id, item) %>%
...
disaggregated <- within |> 
  left_join(between, by = "user_id") |> 
  select(1:10, 12, Internalizing,
         ends_with("_good_between"), ends_with("_able_between"),
         ends_with("_good_within"), ends_with("_able_within"))

### Data plot
data_plot = data_small |> 
  select(bin, daysSinceFirstText, user_id,
         the_good, and_good, to_good, be_good, a_good) |> 
  pivot_longer(cols = everything(), 
               names_to = "word_attribute", 
               values_to = "distance")

data_sx = data_small |> 
  select(bin, daysSinceFirstText, user_id, PHQ, GAD, Internalizing) |> 
  pivot_longer(cols = PHQ:Internalizing, 
               names_to = "Scale", 
               values_to = "Scores")
...
data_small <- data_small %>%
  mutate(
    ...
    Internalizing = scale(Internalizing),
    across(
      c(the_good, and_good, to_good, be_good, a_good, we_good,
        the_able, and_able, to_able, be_able, a_able, we_able),
      scale
    )
  )
...
::: scrollable
```{r}
word_freq = read_csv('/Users/sm9518/Library/CloudStorage/Box-Box/Talkspace_ErikNook_HarvardPrinceton/Data/Processed/Mesquiti_LBA_2024/Talkspace-LBA/I-representations/data/Exploratory_word_frequencies.csv')

word_freq |> 
  mutate(word = str_replace_all(word, " ", "_")) |> 
  filter(word %in% c("i", "the", "and", "to", "be", "a", "we")) |> 
  select(Word = word, Frequency = frequency)  |> 
...
mads_coefs <- mads_coefs |>
  mutate(outcome = recode(outcome,
    "the_good" = "The (Good)",     "the_able" = "The (Able)",
    "and_good" = "And (Good)",     "and_able" = "And (Able)",
    "to_good" = "To (Good)",       "to_able" = "To (Able)",
    "be_good" = "Be (Good)",       "be_able" = "Be (Able)",
    "a_good" = "A (Good)",         "a_able" = "A (Able)",
    "we_good" = "We (Good)",       "we_able" = "We (Able)"
  ))
...
between <- data_small %>%
  gather(item, value,
                  i_good, i_able, 
         and_good, a_good, 
         and_able, a_able,we_good,we_able) %>%
  select(user_id, item, value) %>%
  unique() %>%
  group_by(user_id, item) %>%
  summarize(mean = mean(value, na.rm = TRUE), .groups = "drop") %>%
...
disaggregated <- within |> 
  left_join(between, by = "user_id") |> 
  select(1:10,daysSinceFirstText, 12, Internalizing,
         ends_with("_good_between"), ends_with("_able_between"),
         ends_with("_good_within"), ends_with("_able_within"))
...
```{r}
i_MADS_internalizing <- lmer(Internalizing ~  i_good + daysSinceFirstText + and_good +  (1|room_id), data = data_small)
i_MADS_internalizing <- extract_lmer_data(i_MADS_internalizing)
max_y <- length(unique(i_MADS_internalizing$estimate)) - 0.5
...
```{r}
i_MADS_internalizing <- lmer(Internalizing ~  i_good + daysSinceFirstText  + we_good  + (1|room_id), data = data_small)
i_MADS_internalizing <- extract_lmer_data(i_MADS_internalizing)
max_y <- length(unique(i_MADS_internalizing$estimate)) - 0.5
...
```{r}
i_MADS_internalizing <- lmer(Internalizing ~  i_good + daysSinceFirstText + a_good +  (1|room_id), data = data_small)
i_MADS_internalizing <- extract_lmer_data(i_MADS_internalizing)
max_y <- length(unique(i_MADS_internalizing$estimate)) - 0.5
...
```{r}
i_MADS_internalizing <- lmer(Internalizing ~  i_able + daysSinceFirstText + and_able +  (1|room_id), data = data_small)
i_MADS_internalizing <- extract_lmer_data(i_MADS_internalizing)
max_y <- length(unique(i_MADS_internalizing$estimate)) - 0.5
...
```{r}
i_MADS_internalizing <- lmer(Internalizing ~  i_able + daysSinceFirstText  + we_good  + (1|room_id), data = data_small)
i_MADS_internalizing <- extract_lmer_data(i_MADS_internalizing)
max_y <- length(unique(i_MADS_internalizing$estimate)) - 0.5
...
```{r}
i_MADS_internalizing <- lmer(Internalizing ~  i_able + daysSinceFirstText + a_able +  (1|room_id), data = data_small)
i_MADS_internalizing <- extract_lmer_data(i_MADS_internalizing)
max_y <- length(unique(i_MADS_internalizing$estimate)) - 0.5
...
plots <- lapply(png_files, function(file) {
 ...
 img <- image_read(file) |> 
    image_scale("1000")  # Scale width to 1000px, preserve aspect ratio
...
# Read data
df <- read_csv('/Users/sm9518/Library/CloudStorage/Box-Box/Talkspace_ErikNook_HarvardPrinceton/Data/Processed/Mesquiti_LBA_2024/Talkspace-LBA/I-representations/talkspace_-100-topic_words_frequencies.csv')
...
df <- df |> 
  filter(user_id %in% user_stats$user_id) |> 
  filter(bin < 5)

data_small <- df |> 
  mutate(
    good_MADS = i_good - i_bad,
...
#### create disaggregated data 
between <- data_small %>%
  gather(item, value,happy_MADS,calm_MADS,can_MADS, pct_i,pct_happy,pct_sad,pct_can, emo_pos,emo_neg,tone_pos,tone_neg,n_total) %>%
  select(user_id, item, value) %>%
  unique() %>%
  group_by(user_id, item) %>%
  summarize(mean = mean(value, na.rm = TRUE)) %>%
...
disaggregated <- within |> 
  ...
  select(1:10,12,daysSinceFirstText,
         ...
         pct_can_within,pct_can_between,
         emo_pos_within,emo_pos_between,
         emo_neg_within,emo_neg_between,
         tone_pos_within,tone_pos_between,
         tone_neg_within,tone_neg_between) 
...
ggplot(aes(x = estimate, y = factor(term),
 ...
 model <- lmer(Internalizing ~ daysSinceFirstText + can_MADS_within + can_MADS_between + pct_can_within + pct_can_between +  + emo_pos_within + emo_pos_between + emo_neg_within + emo_neg_between + pct_i_within + pct_i_between + tone_pos_within + tone_pos_between + tone_neg_within + tone_neg_between + n_total_within + n_total_between + (1|room_id), data = disaggregated)
 summary(model)
```
:::
::::
