---
title: "I-Projection Results-Validation"
author: "Steven"
date: "`r Sys.Date()`"
format: 
  revealjs:
...
data = read_csv(file.path(data_dir, "I-representations/data/text-level/MADS_roberta_results_origin_validation_full_pronouns_counts.csv"))
...
data = data %>%
   ...
   dplyr:: mutate(
      ...
      article_valence_combined = rowMeans(select(., a_projection_valence, an_projection_valence, the_projection_valence), na.rm = TRUE),
      article_ability_combined = rowMeans(select(., a_projection_ability, an_projection_ability, the_projection_ability), na.rm = TRUE)
      )
...
between <- good_bad_data_small %>%
  tidyr::gather(item, value,first_person_sing_valence_combined,first_person_sing_ability_combined,pct_good,pct_bad,n_total,tone_pos,tone_neg,pct_able,pct_unable,i
  ) %>%
  ...
  group_by(item) %>%
  mutate(mean = scale(mean, center = TRUE, scale = T)) %>%
  spread(item, mean)

# Within-person variation
within <- good_bad_data_small %>%
  gather(item, value,first_person_sing_valence_combined,first_person_sing_ability_combined,pct_good,pct_bad,n_total,tone_pos,tone_neg,pct_able,pct_unable,i) %>%
  group_by(user_id, item) %>%
  mutate(value = scale(value, center = TRUE, scale = F), # use center=F for within-person since that drops 0s for the numeric variables
...
disaggregated_good_bad <- within |> 
  ...
  select(1:14,daysSinceFirstText,Internalizing,first_person_sing_valence_combined_within,first_person_sing_valence_combined_between,first_person_sing_ability_combined_within, first_person_sing_ability_combined_between,pct_good_between,
         ...
         tone_neg_between,pct_able_within,pct_able_between,pct_unable_within,pct_unable_between,i_between,i_within
  ) 
...
standardize_parameters(internalizing, method = "pseudo", ci_method = "satterthwaite") |> 
  as_tibble() |> 
    rename(
      term = Parameter,
      Std_Coefficient = Std_Coefficient,
...
standardize_parameters(dissagg_proj, method = "pseudo", ci_method = "satterthwaite") |> 
  as_tibble() |> 
    rename(
      term = Parameter,
      Std_Coefficient = Std_Coefficient,
...
standardize_parameters(internalizing_able, method = "pseudo", ci_method = "satterthwaite") |> 
  as_tibble() |> 
    rename(
      term = Parameter,
      Std_Coefficient = Std_Coefficient,
...
standardize_parameters(internalizing_able_disagg, method = "pseudo", ci_method = "satterthwaite") |> 
  as_tibble() |> 
    rename(
      term = Parameter,
      Std_Coefficient = Std_Coefficient,
...
standardize_parameters(pct_words, method = "pseudo", ci_method = "satterthwaite") |> 
  as_tibble() |> 
    rename(
      term = Parameter,
      Std_Coefficient = Std_Coefficient,
...
standardize_parameters(pct_words_disagg, method = "pseudo", ci_method = "satterthwaite") |> 
  as_tibble() |> 
    rename(
      term = Parameter,
      Std_Coefficient = Std_Coefficient,
...
between <- good_bad_data_small %>%
  gather(item, value,
         ...
         article_valence_combined, article_ability_combined,
         ...
         you_projection_valence,you_projection_ability,
         he_projection_valence, he_projection_ability,
         she_projection_valence, she_projection_ability,
         they_projection_valence, they_projection_ability,
...
good_bad_data_small = good_bad_data_small |> 
  mutate(
    ...
    first_person_sing_valence_combined = scale(first_person_sing_valence_combined),
    first_person_sing_ability_combined = scale(first_person_sing_ability_combined),
    article_valence_combined = scale(article_valence_combined), 
    article_ability_combined = scale(article_ability_combined),
    ...
    the_projection_ability = scale(the_projection_ability),
     you_projection_valence = scale(you_projection_valence),
     you_projection_ability = scale(you_projection_ability),
    he_projection_valence = scale(he_projection_valence),
    he_projection_ability = scale(he_projection_ability),
    she_projection_valence = scale(she_projection_valence),
    she_projection_ability = scale(she_projection_ability),
    they_projection_valence = scale(they_projection_valence),
    they_projection_ability = scale(they_projection_ability)
  )
...
"first_person_sing_ability_combined",
"article_valence_combined", 
"article_ability_combined",
"we_projection_valence",
"we_projection_ability",
...
"you_projection_valence",
"you_projection_ability",
"he_projection_valence",
"he_projection_ability",
"she_projection_valence",
"she_projection_ability",
"they_projection_valence",
"they_projection_ability"
)
