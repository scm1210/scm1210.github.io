...
pct_words_disagg <- lmer(Internalizing ~ daysSinceFirstText + first_person_sing_valence_combined_between + first_person_sing_valence_combined_within + 
                        tone_pos_within + tone_pos_between + tone_neg_within + tone_neg_between + n_total_within + n_total_between + i_between + i_within + pct_good_between + pct_good_within + pct_bad_between + pct_bad_within +  (1|room_id), data = disaggregated_good_bad, control =
...
```{r}
pct_words <- lmer(Internalizing ~ i + pct_good + pct_bad + n_total + tone_pos + tone_neg + daysSinceFirstText + first_person_sing_ability_combined + daysSinceFirstText +  (1|room_id), data = good_bad_data_small)
...
data$time_dec <- sapply(strsplit(paste(data$text_time),":"),
                                  function(x) {
                                    x <- as.numeric(x)
                                    (x[1]+x[2]/60+x[3]/3600)/24
                                  }
 )
data$DSFT <- as.numeric(data$daysSinceFirstText + data$time_dec)
...
data_cluster <- data %>%
  select(room_id,4:33,first_person_sing_valence_combined,DSFT) %>%
  unique() %>%
...
combMixModelsLin <- stepFlexmix(first_person_sing_valence_combined ~ DSFT|room_id, k = 1:2, data = data_cluster, nrep=10)
...
cluster_means <- data_cluster %>%
  mutate(cluster_id = bestcombMixModelLin@cluster) %>%
  group_by(cluster_id) %>%
  summarise(mean_val = mean(first_person_sing_valence_combined, na.rm = TRUE)) %>%
  arrange(desc(mean_val))   # use desc() for highest → lowest, remove for lowest → highest

# Extract the order of clusters
ordered_clusters <- cluster_means$cluster_id

# Relabel clusters in the dataset
data_cluster$clusterComb <- factor(
  bestcombMixModelLin@cluster,
  levels = ordered_clusters,
  labels = paste0("Cluster_", seq_along(ordered_clusters))
)
...
plot_aes = theme_minimal() +
  theme(legend.position = "bottom",
        legend.text = element_text(size = 12),
        text = element_text(size = 12, family = "Futura Medium"),
        axis.text = element_text(color = "black",size = 8,angle = 45),
        axis.ticks.y = element_blank())
```
...
fig9a <- ggplot(predicted_df, aes(x = x, y = predicted, color = cluster, fill = cluster, group = cluster)) +
  ...
  theme_minimal() +
  plot_aes +
  theme(axis.text = element_text(size = smallerFontSize-4, color = "black"),
        axis.title = element_text(size = smallerFontSize-2),
...
dw = dw |> 
  mutate(bSx = bPHQ + bGAD,
         fSx = fPHQ + fGAD,
         fdSx = fdPHQ + fdGAD,
         clusterComb = factor(clusterComb, levels = c("Cluster_1", "Cluster_2"))
  )
```
...
fig9d <- ggplot(plotDat_fdSx, aes(x = clusterComb, y = fdSx, fill = clusterComb)) +
  ...
  scale_fill_manual(values = good_palette) +
  xlab("Cluster") +  ylab(expression(Delta~"Symptoms (controlling for baseline)")) +
  theme_minimal() +
  plot_aes +
  theme(axis.text = element_text(size = smallerFontSize-4, color = "black"),
        axis.title = element_text(size = smallerFontSize-2),
...
good_bad_fig
ggsave("/Users/sm9518/Downloads/good_bad_cluster.png", good_bad_fig, width = 10, height = 6, dpi = 300)
...
dw = read_csv("/Users/sm9518/Library/CloudStorage/Box-Box/Talkspace_ErikNook_HarvardPrinceton/Data/Processed/Mesquiti_LBA_2024/Talkspace-LBA/data/TalkSpaceData_wide_exploration.csv")
data_cluster <- data %>%
  select(room_id,4:33,first_person_sing_ability_combined,DSFT) %>%
  unique() %>%
...
able_file_path <- "/Users/sm9518/Library/CloudStorage/Box-Box/Talkspace_ErikNook_HarvardPrinceton/Data/Processed/Mesquiti_LBA_2024/Talkspace-LBA/data/models/1st_person_ability_model.rds"

# Check if the file exists
if (file.exists(able_file_path)) {
  # If it exists, load the dataset
  bestcombMixModelLin <- readRDS(able_file_path)
  message("Data loaded from file.")
} else {

combMixModelsLin <- stepFlexmix(first_person_sing_ability_combined ~ DSFT|room_id, k = 1:5, data = data_cluster, nrep=10)
bestcombMixModelLin <- getModel(combMixModelsLin, "BIC")

# save the model 
  saveRDS(bestcombMixModelLin, file = able_file_path)
  message("Data saved to file.")
}
...
cluster_means <- data_cluster %>%
  mutate(cluster_id = bestcombMixModelLin@cluster) %>%
  group_by(cluster_id) %>%
  summarise(mean_val = mean(first_person_sing_ability_combined, na.rm = TRUE)) %>%
  arrange(desc(mean_val))   # use desc() for highest → lowest, remove for lowest → highest
...
standardize_parameters(clust4, method = "pseudo", ci_method = "satterthwaite")
```
#### Cluster 5

```{r}
clust5 <- lmer(first_person_sing_ability_combined ~ daysSinceFirstText + (1|room_id), data = data_cluster[data_cluster$clusterComb=="Cluster_5",]); 
standardize_parameters(clust5, method = "pseudo", ci_method = "satterthwaite")
```
...
# Extract standardized coefficients for all three clusters
clusters <- list(clust1, clust2, clust3,clust4,clust5)
...
#manually edit cluster 5 label position if needed
mid_points <- mid_points %>%
  mutate(y_max = ifelse(cluster == "5", y_max - 0.025, y_max))
...
dw = dw |> 
  mutate(bSx = bPHQ + bGAD,
         fSx = fPHQ + fGAD,
         fdSx = fdPHQ + fdGAD,
         clusterComb = factor(clusterComb, levels = c("Cluster_1", "Cluster_2", "Cluster_3","Cluster_4","Cluster_5"))
  )
```
...
pw3_sig_labels <- pw3 %>%
  mutate(
    group1 = sub(" - .*", "", contrast),
    group2 = sub(".* - ", "", contrast),
    y.position = min(plotDat_fdSx$fdSx - (plotDat_fdSx$ci %||% 0), na.rm = TRUE) - (row_number() + 1) * 0.3,  # stagger positions
    label = case_when(
      p.value < 0.001 ~ "***",
      p.value < 0.01  ~ "**",
      p.value < 0.05  ~ "*",
...
fig9d <- ggplot(plotDat_fdSx, aes(x = clusterComb, y = fdSx, fill = clusterComb)) +
  ...
  theme_minimal() +
  plot_aes +
  theme(axis.text = element_text(size = smallerFontSize-4, color = "black",angle = 45),
        axis.title = element_text(size = smallerFontSize-2),
        panel.background = element_rect(fill = "white", color = NA)) +
   stat_pvalue_manual(
    pw3_sig_labels, 
...
ggsave("/Users/sm9518/Downloads/able_clsuster.png", able_fig, width = 10, height = 6, dpi = 300)
...
good_bad_data_small = good_bad_data_small |> 
  mutate(
    ...
    and_projection_valence = scale(and_projection_valence),
    and_projection_ability = scale(and_projection_ability),
...
between <- good_bad_data_small %>%
  gather(item, value,
         ...
         and_projection_valence, and_projection_ability,
...
"and_projection_valence",
"and_projection_ability",
...
disagg_plot <- mads_coefs %>%
  ...
  scale_x_continuous(limits = c(-0.10, 0.10), breaks = c(-0.10, 0, 0.10)) +
...
model = lmer(Internalizing ~ daysSinceFirstText + (1|room_id), data = good_bad_data_small_scaled)
model_stan = standardize_parameters(model, method = "pseudo", ci_method = "satterthwaite") 
model_tidy <- broom.mixed::tidy(model, effects = "fixed", conf.int = TRUE, p.values = TRUE)
...
model_merged <- model_tidy %>%
  left_join(model_stan_df, by = "term")


projections <- c("Internalizing")
...
annotation_df <- lapply(projections, function(proj) {
  ...
  # Standardized parameters for this model
  model_stan <- standardize_parameters(model, method = "pseudo", ci_method = "satterthwaite")
  model_stan_df <- as_tibble(model_stan) %>%
    rename(
      term = Parameter,
  ...
  # Extract p-value from the unstandardized model (still needed for significance stars)
  ...
  # Predict fitted value at reference x (using unstandardized model)
  ...
  # Small offset above line (scaled)
...
plot_gam <- data_plot |> 
  ggplot(aes(x = daysSinceFirstText, y = Internalizing)) +
  geom_smooth(
    method = "lm",
    se = TRUE,
    size = 1.5,
    color = good_palette[6],  # line color
    fill = good_palette[6],   # ribbon color
    alpha = 0.25              # transparency for ribbon
  ) +
  labs(y = "Internalizing\nScore", x = "Days in Therapy") +
  plot_aes +
  geom_text(
    data = annotation_df,
    aes(x = 45, y = , label = label),
    inherit.aes = FALSE,
    hjust = 0.5,
    vjust = -0.5,
    size = 12
...
# Plot 2: lmer coefficient plot
plot_coef <- model_merged |> 
  filter(term == "daysSinceFirstText") |>
  mutate(
    term = case_when(
      term == "daysSinceFirstText" ~ "Days in Therapy",
      TRUE ~ term
    )
  ) |>
  ggplot(aes(x = Std_Coefficient, y = term)) +
  geom_pointrange(aes(xmin = CI_low, xmax = CI_high),
                  size = 1.25, linewidth = 1.5, color = good_palette[6]) +
  geom_text(aes(label = case_when(
      p.value < 0.001 ~ "***",
      p.value < 0.01  ~ "**",
      p.value < 0.05  ~ "*",
    ...
    hjust = 0.5, vjust = -0.7, size = 6, color = "black"
  ) +
  labs(
    x = "Standardized Coefficient",
    y = NULL
  ) +
  scale_x_continuous(limits = c(-0.45, 0.1)) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  theme_minimal() +
  theme(
    legend.position = "none",
...
combined_plot <- plot_gam / plot_coef + 
  plot_layout(heights = c(2, 1))  # adjust relative heights

combined_plot

ggsave("/Users/sm9518/Downloads/symptom_plot.png", combined_plot, width = 12, height = 6, dpi = 300)
